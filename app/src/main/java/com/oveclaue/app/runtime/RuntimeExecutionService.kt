...
class RuntimeExecutionService : Service() {
    ...
    private var notificationTitle: String = "OVE Claue is working"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action ?: ACTION_START) {
            ...
            ACTION_COMPLETE -> finishTask(
                ...
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "OVE Claue finished working in $projectName.",
                failed = false,
            )
            ACTION_FAILED -> finishTask(
                title = "Task needs attention",
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "OVE Claue could not finish the task.",
                failed = true,
            )
            ACTION_CANCELLED -> {
                taskRunning = false
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = getSystemService(PowerManager::class.java)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "com.oveclaue.app:active-coding-task")
    ...
    companion object {
        const val ACTION_START = "com.oveclaue.app.START_RUNTIME"
        const val ACTION_STOP = "com.oveclaue.app.STOP_RUNTIME"
        const val ACTION_PROGRESS = "com.oveclaue.app.PROGRESS_RUNTIME"
        const val ACTION_COMPLETE = "com.oveclaue.app.COMPLETE_RUNTIME"
        const val ACTION_FAILED = "com.oveclaue.app.FAIL_RUNTIME"
        const val ACTION_CANCELLED = "com.oveclaue.app.CANCEL_RUNTIME"
        ...
        fun ensureNotificationChannels(context: android.content.Context) {
            ...
            manager.createNotificationChannel(
                NotificationChannel(RUNNING_CHANNEL_ID, "Running coding tasks", NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Shows progress while OVE Claue is working in the background"
                },
            )
            manager.createNotificationChannel(
