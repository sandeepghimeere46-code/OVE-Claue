package com.oveclaue.app.model
...
enum class ProviderKind(
    ...
    LOCAL_LLAMA("Local Offline AI", "Runs completely on-device (~750MB)", ProviderProtocol.OPENAI_CHAT, "http://127.0.0.1:8080/v1", "qwen2.5", true, true, true),
...
fun defaultDshApiForProvider(kind: ProviderKind): String = when (kind) {
    ...
    ProviderKind.LOCAL_LLAMA -> "openai-completions"
...
enum class DevStack(
    ...
    LOCAL_AI(
        "Local Offline AI Server",
        "On-device inference (~750 MB). Runs fully offline.",
        "llama-server and Qwen2.5-0.5B GGUF"
    ),
    PHP(
        "PHP",
